...
`/people/[slug]` and `/cemeteries/[slug]` are real, working pages now — the "View profile" links from the explore sheet, Nearby cards, and cemetery notable-people lists all resolve instead of 404ing. A person page states plainly whether its map marks an exact grave or only the cemetery, same wording as Explore; "why they're interesting" and biography sections only render when that research actually exists (currently none of it does — see docs/DATA-SOURCES.md), rather than showing an empty heading. Every source citation is a real, clickable link with a retrieval date, not a vague "sourced" badge. A cemetery page lists every notable person there, sorted by Dead Score, and the categories actually represented among them — derived from its people, not maintained as a separate hand-picked list that could drift out of sync.

## Next experience

Navigation: Explore, Search, Nearby, Saved (future placeholder) — Explore and Nearby exist as real routes today; profile pages exist as destinations but have no dedicated nav entry of their own (visitors arrive at them from Explore, Nearby, or another profile's "nearby" list). Search still needs a real route. Exact coordinates do not establish public access; cemetery visiting rules still apply.
