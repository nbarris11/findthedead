...
Queries start from each state's geographic bounds. A response hitting 2,000 rows is subdivided and retried so a query limit is not mistaken for complete coverage. Very dense locations that still hit the limit at maximum depth switch to ordered pagination. Earlier checkpoints with capped boxes are reopened automatically on resume. Interrupted/failed boxes remain pending, and the process exits unsuccessfully if the requested state searches are incomplete. A completed search means the planned API searches finished without a detected cap, not that Wikidata contains every U.S. burial. Changes made upstream during collection can still affect paginated results; use a new snapshot for later refreshes.
